...
// Trainerize is the source of truth for who is assigned to which coach. Each
// active client record carries trainerID + details.trainer (the coach), so we
// build the full roster here — including clients who are not in GymMaster.
async function buildTzRoster(){
  const roster=[]; const seen=new Set();
  for(const start of [0,1000,2000]){
    const d=await api('/user/getClientList',{view:'activeClient',start,count:1000,verbose:true});
    const users=(d&&d.users)||[];
    for(const u of users){
      if(seen.has(u.id)) continue; seen.add(u.id);
      const tr=(u.details&&u.details.trainer)||{};
      const coach=(tr.firstName||'').trim()||null;
      roster.push({
        tzId:u.id,
        firstName:u.firstName||'', surname:u.lastName||'',
        name:`${u.firstName||''} ${u.lastName||''}`.trim(),
        email:String(u.email||'').toLowerCase(),
        trainerId:u.trainerID||tr.id||null,
        coach,
      });
    }
    if(users.length<1000) break;
  }
  const byEmail={}, byTzId={}, counts={};
  for(const r of roster){
    if(r.email) byEmail[r.email]=r.coach;
    byTzId[String(r.tzId)]=r.coach;
    if(r.coach) counts[r.coach]=(counts[r.coach]||0)+1;
  }
  const H={apikey:SB_KEY,Authorization:`Bearer ${SB_KEY}`,'Content-Type':'application/json'};
  const body=JSON.stringify({updated:new Date().toISOString(),count:roster.length,counts,roster,byEmail,byTzId});
  await fetch(`${SB_URL}/rest/v1/invoices?source=eq.tz-roster`,{method:'DELETE',headers:{...H,Prefer:'return=minimal'}});
  await fetch(`${SB_URL}/rest/v1/invoices`,{method:'POST',headers:{...H,Prefer:'return=minimal'},body:JSON.stringify({trainer_name:'__tz-roster__',period_start:today,period_end:today,source:'tz-roster',status:'data',raw_text:body,created_by:'weekly-sync'})});
  return {count:roster.length,counts};
}
export default async()=>{
  if(!GROUP_ID||!TOKEN||!SB_URL||!SB_KEY) return new Response(JSON.stringify({ok:false,error:'missing env'}),{status:500});
  try{ try{ globalThis.__tzRoster=await buildTzRoster(); }catch(e){ console.log('tz roster failed',e); } const DATA=await buildDATA(); await sbWrite(JSON.stringify(DATA));
